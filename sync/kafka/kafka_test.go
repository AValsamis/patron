...
func TestNewSyncProducer_Success(t *testing.T) {
	...
	got, err := NewSyncProducer([]string{seed.Addr()}, Version(sarama.V0_8_2_0.String()))
	assert.NoError(t, err)
	assert.NotNil(t, got)
}
...
func TestSyncProducer_SendMessage_Close(t *testing.T) {
	...
	assert.NoError(t, err)
	seed := createKafkaBroker(t, false)
	ap, err := NewSyncProducer([]string{seed.Addr()}, Version(sarama.V0_8_2_0.String()))
	assert.NoError(t, err)
	assert.NotNil(t, ap)
